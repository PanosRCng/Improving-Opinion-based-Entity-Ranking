...
// #######################################################################
	...
	if (results.is_open())
	{
		...
		//	clusters = kmeans->do_kmeans("features", 10);
			...
			// for every query use 5 disambiguation queries and for every disambiguation query get a document ranking list using BM25
			...
			// do the ClustFuse algorithm using the Kurland object to give a score to every document -- same variable names as the paper
