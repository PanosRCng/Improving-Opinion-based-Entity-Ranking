...
//	string Dinf = string(project_path + "D_inf_2007");
...
/*
	...
	for(int i=0; i<dinf_folders.size(); i++)
	{
		...
		for(int j=0; j<5; j++)
		{
			string j_s = static_cast<ostringstream*>( &(ostringstream() << j) )->str();

			string file = string(folder + "/" + j_s);
			dinf_files.push_back(file); 
		}
	}
	cout << "get all Dinf files: " << dinf_files.size() << endl;

	for(int i=0; i<dinf_files.size(); i++)
	{
		cout << dinf_files.at(i) << endl;
	}
*/
